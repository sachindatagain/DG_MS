...
#     try:
#         # Query to fetch distinct values
#         query = f"SELECT DISTINCT `{column_name}` FROM managed_service WHERE `{column_name}` IS NOT NULL"
#         cursor.execute(query)
#         results = cursor.fetchall()
...
#     # Base query
#     query = "SELECT * FROM managed_service WHERE 1=1"
#     parameters = []
...
#     # Filters
#     email_address = st.selectbox("Email Address", options=[""] + fetch_unique_values("email_address"))
#     OCRs = st.multiselect("OCR", options=fetch_unique_values("OCR"))
#     payers = st.multiselect("Payers", options=fetch_unique_values("payer"))
#     bill_ref_code = st.text_input("Bill Reference Code")
#     track_id = st.text_input("Track ID")
...
#             Annotation_id=Annotation_id if Annotation_id else None
#         )
#         if not (email_address or OCRs or payers or start_date or end_date or bill_ref_code or track_id):
#           st.warning("Select any one of the sections to filter data.")
...
#             # Reset index to start from 1 instead of 0
#             df.index = range(1, total_records + 1)
